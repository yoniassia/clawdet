...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/health' || req.url === '/v1/health') {
    ...
    res.end(JSON.stringify({ status: 'ok', engine: 'nanoclaw', tools: true, uptime: process.uptime() }));
    return;
  }
  ...
  if (req.url === '/v1/chat/completions' && req.method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        ...
        const reply = await chatWithTools(messages, model);
        res.writeHead(200, { 'Content-Type': 'application/json' });
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`NanoClaw agent on port ${PORT} (tools enabled)`);
